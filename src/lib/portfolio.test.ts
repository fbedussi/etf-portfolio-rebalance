...
import { calculateCurrentAssetClassAllocation, calculateCurrentEtfData, calculateCurrentPortfolioValue, calculateCurrentPortfolioValueForCountryAllocation, calculateCurrentValuesByCountry, calculatePortfolioCost, getDriftDataByAssetClass, pricesHistoryToMap, quantityAtDate } from './portfolio.ts';
...
describe('quantityAtDate with SIP', () => {
    test('the same day of the start date', () => {
        assert.strictEqual(quantityAtDate([], '2020-01-01', { quantity: 1, dayOfMonth: 1, frequency: 12, startDate: '2020-01-01' }), 1)
    })
    test('the same day of the start date with more than one quantity', () => {
        assert.strictEqual(quantityAtDate([], '2020-01-01', { quantity: 2, dayOfMonth: 1, frequency: 12, startDate: '2020-01-01' }), 2)
    })
    test('the day after the start date', () => {
        assert.strictEqual(quantityAtDate([], '2020-01-02', { quantity: 1, dayOfMonth: 1, frequency: 12, startDate: '2020-01-01' }), 1)
    })
    test('the day after the start date with more than one quantity', () => {
        assert.strictEqual(quantityAtDate([], '2020-01-02', { quantity: 2, dayOfMonth: 1, frequency: 12, startDate: '2020-01-01' }), 2)
    })
    test('the same day of the second month', () => {
        assert.strictEqual(quantityAtDate([], '2020-02-01', { quantity: 1, dayOfMonth: 1, frequency: 12, startDate: '2020-01-01' }), 2)
    })
    test('the same day of the second month with more than one quantity', () => {
        assert.strictEqual(quantityAtDate([], '2020-02-01', { quantity: 2, dayOfMonth: 1, frequency: 12, startDate: '2020-01-01' }), 4)
    })

    test('the same day of the first month of the previous year', () => {
        assert.strictEqual(quantityAtDate([], '2020-01-01', { quantity: 1, dayOfMonth: 1, frequency: 12, startDate: '2019-01-01' }), 13)
    })
    test('the same day of the first month of the previous year with more than one quantity', () => {
        assert.strictEqual(quantityAtDate([], '2020-01-01', { quantity: 2, dayOfMonth: 1, frequency: 12, startDate: '2019-01-01' }), 26)
    })
    test('the day after the same day of the first month of the previous year', () => {
        assert.strictEqual(quantityAtDate([], '2020-01-02', { quantity: 1, dayOfMonth: 1, frequency: 12, startDate: '2019-01-01' }), 13)
    })
    test('the day after the same day of the first month of the previous year with more than one quantity', () => {
        assert.strictEqual(quantityAtDate([], '2020-01-02', { quantity: 2, dayOfMonth: 1, frequency: 12, startDate: '2019-01-01' }), 26)
    })
    test('the same day of the second month of the previous year', () => {
        assert.strictEqual(quantityAtDate([], '2020-02-01', { quantity: 1, dayOfMonth: 1, frequency: 12, startDate: '2019-01-01' }), 14)
    })
    test('the same day of the second month of the previous year with more than one quantity', () => {
        assert.strictEqual(quantityAtDate([], '2020-02-01', { quantity: 2, dayOfMonth: 1, frequency: 12, startDate: '2019-01-01' }), 28)
    })
})
...
describe('calculatePortfolioCost', () => {
    ...
    test('returns 0 if etf has no transactions', () => {
        assert.strictEqual(calculatePortfolioCost({
            'isin1': etf1
        }, {}, '2020-01-01'), 0)
    })
        ...
        assert.strictEqual(calculatePortfolioCost(
            {
                'isin1': {
                    ...etf1,
                    transactions: [
                        { quantity: 10, price: 101 } as Transaction,
                    ],
                    sip: {
                        quantity: 10,
                        dayOfMonth: 1,
...
describe('calculateCurrentPortfolioValue', () => {
    ...
    test('returns 0 if etf has no transactions', () => {
        assert.strictEqual(calculateCurrentPortfolioValue({
            'isin1': etf1
        }, { 'isin1': { price: 100, timestamp: '2020-01-01', history: [] } }, '2020-01-01'), 0)
    })
    ...
    test('returns the value of the transactions', () => {
        ...
        }, { 'isin1': { price: 100, timestamp: '2020-01-01', history: [] } }, '2020-01-01'), 10 * 100)
    })
    ...
    test('returns the value of the transactions of multiple etfs', () => {
        assert.strictEqual(calculateCurrentPortfolioValue({
            'isin1': {
                ...etf1,
                transactions: [
                    { quantity: 10, price: 50 } as Transaction,
                ]
            },
            'isin2': {
                ...etf1,
                isin: 'isin2',
                transactions: [
                    { quantity: 5, price: 50 } as Transaction,
                ]
            }
        }, {
...
describe('calculateCurrentPortfolioValueForCountryAllocation', () => {
    ...
    test('returns 0 if no etfs', () => {
        assert.strictEqual(calculateCurrentPortfolioValueForCountryAllocation({}, prices, '2020-01-01'), 0)
    })
    ...
    test('returns value for stock ETFs', () => {
        ...
        assert.strictEqual(calculateCurrentPortfolioValueForCountryAllocation(etfs, prices, '2020-01-01'), 1000)
    })
    ...
    test('ignores non-stock ETFs', () => {
        ...
        assert.strictEqual(calculateCurrentPortfolioValueForCountryAllocation(etfs, prices, '2020-01-01'), 0)
    })
...
describe('calculateCurrentAssetClassAllocation', () => {
    ...
    test('calculates allocation for multiple ETFs with different asset classes', () => {
        ...
        assert.deepStrictEqual(calculateCurrentAssetClassAllocation(etfs, prices2, '2020-01-01', 2000), {
            'stocks': 50,
            'bonds': 50
        })
    })
